...
export PRODUCT_BUNDLE_IDENTIFIER = "dev.ploosh.libhooker-oss"
LHC_ID   = dev.ploosh.libhooker-configurator
...
$(LHC_APP_DIR):
	@set -o pipefail; \
		xcodebuild build -jobs $(shell sysctl -n hw.ncpu) -project 'libhooker configurator.xcodeproj' -scheme 'libhooker configurator' -configuration $(BUILD_CONFIG) -arch $(ARCH) -sdk $(PLATFORM) -derivedDataPath $(LHCTMP) \
		...
		OTHER_SWIFT_FLAGS="$(CFLAGS)" DSTROOT=$(LHCTMP)/install $(XCPRETTY)
